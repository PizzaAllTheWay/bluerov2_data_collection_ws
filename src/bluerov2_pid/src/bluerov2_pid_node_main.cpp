...
int main(int argc, char * argv[])
{
  ...
  //   }
    ...
    // Create hysteretic controller node
    const auto bluerov2_pid_ptr =
      std::make_shared<bluerov2_pid::PIDControllerNode>("bluerov2_pid");

    exec.add_node(bluerov2_pid_ptr->get_node_base_interface());
    ...
    //   pendulum::utils::autostart(*bluerov2_pid_ptr);
    // }

    exec.spin();
    rclcpp::shutdown();
  } catch (const std::exception & e) {
    RCLCPP_INFO(rclcpp::get_logger("bluerov2_pid"), "%s", e.what());
    ret = 2;
  } catch (...) {
    RCLCPP_INFO(
      rclcpp::get_logger("bluerov2_pid"), "Unknown exception caught. "
      "Exiting...");
    ret = -1;
  }
  return ret;
